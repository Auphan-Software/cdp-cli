...
export class OperationLeaseManager {
  ...
  /** Atomically replace a live lease held by the same owner for forced reset. */
  replaceOwned(rootTargetId: string, owner: string, ttlMs = this.defaultTtlMs): OperationLease {
    ...
    const lease: OperationLease = {
      leaseId: this.leaseId(), rootTargetId, owner,
      acquiredAt: now, lastHeartbeatAt: now, ttlMs
    };
    this.leases.set(rootTargetId, lease);
    return cloneLease(lease);
  }
