  ...
  const browser = {
    ...
    async send(method: string, params: Record<string, unknown> = {}) {
      ...
      if (method === 'Target.createTarget') {
        targetId += 1;
        if (targetId === options.failCreateTargetAt) throw new Error('synthetic replacement failure');
      ...
      if (method === 'Target.disposeBrowserContext') {
        for (const target of registry.list()) {
          if (target.browserContextId === params.browserContextId) registry.removeTarget(target.targetId);
        }
      }
      return {};
    },
...
describe('WorkspaceSessionService', () => {
  it('ensures one stable primary page without multiplying targets on rerun', async () => {
    ...
    const first = await service.ensureSession('agent-42', { url: 'https://example.test' });
    const second = await service.ensureSession('agent-42', { url: 'https://ignored.test' });

    expect(first.created).toBe(true);
    expect(second.created).toBe(false);
    expect(second.pageId).toBe(first.pageId);
    expect(second.session.pageIds).toEqual([first.pageId]);
    expect(methods.filter((method) => method === 'Target.createTarget')).toHaveLength(1);
    service.close();
  });
  ...
  it('resets only the named session, disposes its old context, and creates one replacement', async () => {
    const fs = new MemoryFileSystem();
    const store = new SessionStore('sessions.json', { fileSystem: fs, temporaryId: () => String(fs.files.size) });
    const { browser, calls } = fakeBrowser();
    const service = await WorkspaceSessionService.open('http://localhost:9222', { browser, store });
    ...
    expect(service.listSessions().map((session) => session.name).sort()).toEqual(['agent-1154', 'other-agent']);
  ...
  it('refuses to reset a shared session', async () => {
    const fs = new MemoryFileSystem();
    const store = new SessionStore('sessions.json', { fileSystem: fs, temporaryId: () => String(fs.files.size) });
    const { browser } = fakeBrowser();
    const service = await WorkspaceSessionService.open('http://localhost:9222', { browser, store });
  ...
  it('reports an incomplete reset when replacement creation fails', async () => {
    const fs = new MemoryFileSystem();
    const store = new SessionStore('sessions.json', { fileSystem: fs, temporaryId: () => String(fs.files.size) });
    const { browser } = fakeBrowser({ failCreateTargetAt: 2 });
    const service = await WorkspaceSessionService.open('http://localhost:9222', { browser, store });
    ...
    await expect(service.resetSession('agent-1154', { force: true })).rejects.toMatchObject({
      ...
      details: { sessionName: 'agent-1154', affectedPageIds: ['page-1'] }
    });
    expect(service.listSessions()).toEqual([]);
    service.close();
  });
